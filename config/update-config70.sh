...
jsonfile=$(jq ".os.sha256=\"$value\"" $model/7.0.1-42218/config.json) 
echo $jsonfile | jq . > $model/7.0.1-42218/config.json

value=`grep $model _common/kernel-bs-fb-patch-for-all/files-chksum | grep files.zlinux.sha256|awk '{print $1}'`
jsonfile=$(jq ".files.zlinux.sha256=\"$value\"" $model/7.0.1-42218/config.json) 
echo $jsonfile | jq . > $model/7.0.1-42218/config.json

value=`grep $model _common/kernel-bs-fb-patch-for-all/files-chksum | grep files.ramdisk.sha256|awk '{print $1}'`
jsonfile=$(jq ".files.ramdisk.sha256=\"$value\"" $model/7.0.1-42218/config.json) 
echo $jsonfile | jq . > $model/7.0.1-42218/config.json

value=`grep $model _common/kernel-bs-fb-patch-for-all/files-chksum | grep files.vmlinux.sha256|awk '{print $1}'`
jsonfile=$(jq ".files.vmlinux.sha256=\"$value\"" $model/7.0.1-42218/config.json) 
echo $jsonfile | jq . > $model/7.0.1-42218/config.json
...
echo "$remodel"
cp _common/kernel-bs-fb-patch-for-all/zImage-001-${model}-42218-ramdisk-and-flag-NOP.bsp $model/7.0.1-42218/zImage-001-${remodel}-42218-ramdisk-and-flag-NOP.bsp
