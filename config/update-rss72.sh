...
while read -r model; do
  ...
  echo "md5: $value"
  jq --arg remodel "$remodel" --arg value "$value" '.channel.item[] |= if .BuildNum == 64570 then .model |= map(if .mUnique | contains($remodel) then .mCheckSum = $value else . end) else . end' ../rss/7.2.0/rss.json > ../rss_tmp.json
  mv ../rss_tmp.json ../rss/7.2.0/rss.json
done < models

# convert rss.xml from rss.json
tce-load -wi python
python2 convert_rss_xml72.py
